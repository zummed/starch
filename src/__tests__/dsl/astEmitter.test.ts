...
import { flattenLeaves, nodeAt } from '../../dsl/astTypes';
...
// ─── Parity Tests with Existing Generator ────────────────────────

describe('astEmitter - parity with existing generator', () => {
  const parityScenes: Array<{ name: string; scene: any; nodeFormats?: Record<string, 'inline' | 'block'> }> = [
    // Basic geometry
    ...
    { name: 'layout block', scene: { objects: [{ id: 'box', rect: { w: 10, h: 10 }, layout: { type: 'flex', direction: 'row', gap: 10 } }] } },
    ...
    // Block mode
    { name: 'force block', scene: { objects: [{ id: 'dot', ellipse: { rx: 5, ry: 5 }, fill: { h: 0, s: 80, l: 50 } }] }, nodeFormats: { dot: 'block' } },
    { name: 'force inline many props', scene: { objects: [{ id: 'box', rect: { w: 160, h: 100 }, fill: { h: 210, s: 70, l: 45 }, stroke: { color: 'black' }, opacity: 0.8, depth: 5 }] }, nodeFormats: { box: 'inline' } },
  ...
  for (const { name, scene, nodeFormats } of parityScenes) {
    it(`matches generator: ${name}`, () => {
      const opts = nodeFormats ? { nodeFormats, formatHints: hints } : { formatHints: hints };
      const expected = generateDsl(scene, opts);
      const { text } = buildAstFromModel(scene, hints, nodeFormats);
