...
describe('applyEffects', () => {
  ...
  it('flash dims opacity temporarily', () => {
    const effects: EffectInstance[] = [
      { target: 'b', effect: 'flash', amplitude: 0.5, triggerTime: 0 },
    ];
    const result = makeResult('b', { opacity: 1 });
    applyEffects(effects, result, 0.2); // peak
    expect(result.b.opacity as number).toBeLessThan(1);
    expect(result.b.opacity as number).toBeGreaterThan(0);
  });
  ...
  it('shake adds deterministic offset to x/y', () => {
    ...
    // Use t=0.05 to avoid sine zero crossings
    applyEffects(effects, result1, 0.05);
    applyEffects(effects, result2, 0.05);
    // Same time = same result (deterministic)
